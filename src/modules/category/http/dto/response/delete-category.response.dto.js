const { z } = require("zod");

/**
 * Schema Zod para a resposta de exclusão de categoria.
 * Filtra apenas os campos seguros para exposição na API.
 * @type {import('zod').ZodObject}
 */
const deleteCategoryResponseSchema = z.object({
  id: z.uuid(),
  name: z.string(),
  slug: z.string(),
  use_in_menu: z.coerce.boolean(),
});

/**
 * DTO de resposta para exclusão de categorias.
 * Garante que apenas os campos definidos no schema sejam retornados.
 */
const DeleteCategoryResponseDto = {
  /**
   * Transforma o payload da categoria deletada em resposta segura.
   * @param {Object} payload - Dados brutos da categoria (model Sequelize).
   * @returns {Object} Dados filtrados pelo schema Zod.
   */
  toResponse(payload) {
    return deleteCategoryResponseSchema.parse(payload);
  },
};

module.exports = DeleteCategoryResponseDto;
